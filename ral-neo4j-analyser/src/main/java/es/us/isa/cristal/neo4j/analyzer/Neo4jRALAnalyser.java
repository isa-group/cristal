...
import es.us.isa.cristal.neo4j.analyzer.operations.AllActivitiesOP;
...
import es.us.isa.cristal.neo4j.analyzer.operations.IndispensableParticipantsOP;
...
public class Neo4jRALAnalyser implements RALAnalyser {
	...
	public Set<String> allActivities(TaskDuty duty) {
		Operation<Set<String>> op = new AllActivitiesOP(bpEngine, resolver, processInstanceId, duty);
		return op.execute();
	}
	...
	public Set<String> indispensableParticipants(Iterable<String> activities,
			TaskDuty duty) {
		Operation<Set<String>> op = new IndispensableParticipantsOP(bpEngine, resolver, processInstanceId, activities, duty);
		return op.execute();
	}
